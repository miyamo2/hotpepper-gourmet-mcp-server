...
var (
	...
	ToolDinnerBudgetMasterSearchInputSchema     = json.RawMessage(`{"$schema":"https://json-schema.org/draft/2020-12/schema","properties":{"start":{"type":"integer","title":"検索の開始位置","description":"検索結果の何件目から出力するかを指定します。初期値:1"},"count":{"type":"integer","title":"1ページあたりの取得数","description":"検索結果の最大出力データ数を指定します。初期値：なし。最小: 1、最大：なし（すべて取得）、推奨: なし（すべて取得）"}},"additionalProperties":false,"type":"object"}`)
	ToolLargeServiceAreaMasterSearchInputSchema = json.RawMessage(`{"$schema":"https://json-schema.org/draft/2020-12/schema","properties":{"start":{"type":"integer","title":"検索の開始位置","description":"検索結果の何件目から出力するかを指定します。初期値:1"},"count":{"type":"integer","title":"1ページあたりの取得数","description":"検索結果の最大出力データ数を指定します。初期値：なし。最小: 1、最大：なし（すべて取得）、推奨: なし（すべて取得）"}},"additionalProperties":false,"type":"object"}`)
	ToolServiceAreaMasterSearchInputSchema      = json.RawMessage(`{"$schema":"https://json-schema.org/draft/2020-12/schema","properties":{"start":{"type":"integer","title":"検索の開始位置","description":"検索結果の何件目から出力するかを指定します。初期値:1"},"count":{"type":"integer","title":"1ページあたりの取得数","description":"検索結果の最大出力データ数を指定します。初期値：なし。最小: 1、最大：なし（すべて取得）、推奨: なし（すべて取得）"}},"additionalProperties":false,"type":"object"}`)
